...
def load_folds_data(np_data_path, n_folds, split_ratio=0.1):
    files = sorted(glob(os.path.join(np_data_path, "*.npz")))
    ...
    all_r_permute = list(range(len(files)))
    random.seed(1004)
    random.shuffle(all_r_permute)
    r_permute = all_r_permute[: int((1-split_ratio) * len(all_r_permute))]
    test_permute = all_r_permute[int((1-split_ratio) * len(all_r_permute)):]
    files_dict = dict()

    # Save test file index. 
    with open("test_indices.txt", "w") as f:
        test_permute = [f"{x}," for x in test_permute]
        f.writelines(test_permute)
...
def calc_class_weight(labels_count, class_importance = [1, 2.6, 1.3, 1]):
    total = np.sum(labels_count)
    class_weight = dict()
    num_classes = len(labels_count)
    assert num_classes == len(class_importance),"class_importance의 개수는 Class의 개수와 일치해야 합니다."

    factor = 1 / num_classes
    mu = [factor * importance for importance in class_importance]
